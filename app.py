...
# ============================================
# CrediFast - APP de Risco de Crédito (Completo + Rápido)
# - Modo Rápido (default) e Modo Completo (opcional)
# - Fallbacks de bibliotecas (SHAP/XGB/LightGBM/SMOTE)
# - Cache, botões e subsets para desempenho
# ============================================
...
if not HAS_SMOTE:
    st.caption("⚠️ SMOTE indisponível (imbalanced-learn). O app funciona, mas o treino pode seguir sem balanceamento.")
if not HAS_XGB:
    st.caption("⚠️ XGBoost indisponível. Use Modo Rápido ou Completo sem XGB.")
if not HAS_LGBM:
    st.caption("⚠️ LightGBM indisponível. Use Modo Rápido ou Completo sem LGBM.")
if not HAS_SHAP:
    st.caption("⚠️ SHAP indisponível. A explicabilidade usará Permutation Importance (fallback).")
...
def build_preprocessor(df, target_col="loan_status"):
    """Cria ColumnTransformer com imputação/escala/one-hot."""
...
def evaluate_model(name, model, X_train, y_train, X_test, y_test, plot_roc=True):
    ...
    return {"name": name, "model": model, "auc": auc, "accuracy": acc, "precision": prec, "recall": rec, "f1": f1, "cm": cm}, fig

@st.cache_data
def cached_evaluate(name, model, X_train, y_train, X_test, y_test):
    """Cache para não recalcular sempre que trocar de aba."""
    return evaluate_model(name, model, X_train, y_train, X_test, y_test, plot_roc=True)

def get_feature_names(preprocessor, num_cols, cat_cols):
    """Nomes pós-OneHot para usar no SHAP ou permutation importance."""
...
with tabs[1]:
    ...
    if mode.startswith("Rápido"):
        ...
        # Completo: inclui todo o conjunto pedido
        models = [
            ("KNN", None),  # adicionado adiante se necessário
            ("SVM", SVC(kernel='rbf', probability=True, random_state=RANDOM_STATE)),
            ("DecisionTree", DecisionTreeClassifier(max_depth=6, random_state=RANDOM_STATE)),
        ...
        # KNN separado (melhor com dados já escalados; aqui já estão)
    ...
    results_df = pd.DataFrame([{
        "name": r["name"], "auc": r["auc"], "accuracy": r["accuracy"],
    ...
    # Persistência & download
    save_art = st.checkbox("Salvar artefatos (preprocessor + best_model)")
    if save_art:
...
with tabs[2]:
    ...
    if HAS_SHAP and best_name in ["RandomForest", "GradientBoosting"] + (["XGBoost", "LightGBM"] if HAS_XGB or HAS_LGBM else []):
        # SHAP TreeExplainer
        try:
            ...
            # Exemplo local: um bad e um good
            ...
            st.info("Fallback para Permutation Importance abaixo.")
